pipeline {
    environment {
      ...
      namespace = "${env.branchname == 'development' ? 'googleclass-dev' : env.branchname == 'release' ? 'googleclass-hom' : env.branchname == 'homolog-r2' ? 'googleclass-hom2' : 'sme-googleclass' }"
    }
                ...
                script{
                    if ( env.branchname == 'main' ||  env.branchname == 'master' || env.branchname == 'homolog' || env.branchname == 'release' ) {
                        ...
                        timeout(time: 24, unit: "HOURS") {
                            input message: 'Deseja realizar o deploy?', ok: 'SIM', submitter: 'robson_silva, marlon_goncalves, rafael_losi, ricardo_coda, felipe_abreu, juliano_campos'
                        }
                    }
