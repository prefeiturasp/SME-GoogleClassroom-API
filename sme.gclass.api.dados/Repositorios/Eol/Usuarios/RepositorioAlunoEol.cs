...
namespace SME.GoogleClassroom.Dados
{
    public class RepositorioAlunoEol : RepositorioEol, IRepositorioAlunoEol
    {
		...
		public async Task<IEnumerable<long>> ObterAlunosCodigosInativosPorAnoLetivoETurma(int anoLetivo, long turmaId, DateTime dataReferencia, int totalDiasInativo)
		{
			using var conn = ObterConexao();
			...
			const string query = @"
				...
				WHERE
					...
					AND matr.dt_status_matricula + @totalDiasInativo <= @dataReferencia 
				    AND matr.dt_status_matricula + @totalDiasInativo <= GETDATE()
			...
			return await conn.QueryAsync<long>(query, new { turmaId, anoLetivo, dataReferencia, totalDiasInativo });
		}
