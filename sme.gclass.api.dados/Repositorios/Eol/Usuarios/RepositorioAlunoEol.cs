...
namespace SME.GoogleClassroom.Dados
{
    public class RepositorioAlunoEol : RepositorioEol, IRepositorioAlunoEol
    {
        ...
        public async Task<IEnumerable<AlunoCelpDto>> ObterAlunosMatriculadosCelpPorComponenteEAnoLetivo(int componente, int anoLetivo)
        {
	        const string query = @"select amn.CodigoAluno,
				...
				where gcc.cd_componente_curricular = @componente
				and te.an_letivo = @anoLetivo
				and e.tp_escola = 27
				and amn.CodigoSituacaoMatricula in (1, 2, 3, 6, 10, 13)";

	        using var conn = ObterConexao();
	        return await conn.QueryAsync<AlunoCelpDto>(query, new { componente, anoLetivo });
        }
    }
}
