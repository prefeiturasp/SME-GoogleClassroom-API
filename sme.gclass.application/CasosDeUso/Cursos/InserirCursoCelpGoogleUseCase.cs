...
{
    public class InserirCursoCelpGoogleUseCase : IIncluirCursoCelpUseCase
    {
        ...
        {
            await IniciarSyncGoogleProfessoresDoCursoCelpAsync(cursoGoogle);
            
            await IniciarSyncGoogleAlunosDoCursoCelpAsync(cursoGoogle);
            
            await IniciarSyncGoogleFuncionariosDoCursoCelpAsync(cursoGoogle);
        }

        private async Task IniciarSyncGoogleProfessoresDoCursoCelpAsync(CursoGoogle cursoGoogle)
        {
            var publicarCursosDoProfessor = await mediator.Send(new PublicaFilaRabbitCommand(RotasRabbit.FilaCursoProfessorCelpSync, RotasRabbit.FilaCursoProfessorCelpSync, cursoGoogle));
        ...
        private async Task IniciarSyncGoogleAlunosDoCursoCelpAsync(CursoGoogle cursoGoogle)
        {
            var publicarCursosDoProfessor = await mediator.Send(new PublicaFilaRabbitCommand(RotasRabbit.FilaCursoAlunoCelpSync, RotasRabbit.FilaCursoAlunoCelpSync, cursoGoogle));
            if (!publicarCursosDoProfessor)
            {
                await mediator.Send(new InserirCursoErroCommand(cursoGoogle.TurmaId, cursoGoogle.ComponenteCurricularId,
