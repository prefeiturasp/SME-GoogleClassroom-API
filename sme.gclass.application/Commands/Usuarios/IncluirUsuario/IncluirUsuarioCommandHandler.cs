...
namespace SME.GoogleClassroom.Aplicacao
{
    public class IncluirUsuarioCommandHandler : IRequestHandler<IncluirUsuarioCommand, long>
    {
        ...
        public async Task<long> Handle(IncluirUsuarioCommand request, CancellationToken cancellationToken)
            => await repositorioUsuario.SalvarAsync(request.Id, request.Cpf, request.Nome, request.Email, request.Tipo, request.OrganizationPath, request.DataInclusao, request.DataAtualizacao, request.GoogleClassroomId);
    }
}